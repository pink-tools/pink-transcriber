...
func main() {
	...
	case "stop":
		if err := daemon.Stop(baseDir); err != nil {
			otel.Error("stop failed", map[string]any{"error": err.Error()})
			os.Exit(1)
		}
		otel.Info("stopped")
	case "status":
	...
	case "transcribe":
		if len(os.Args) < 3 {
			otel.Error("missing file argument")
...
func runDaemon(baseDir, whisperDir string) {
	if err := bootstrap.EnsureReady(); err != nil {
		otel.Error("bootstrap failed", map[string]any{"error": err.Error()})
		os.Exit(1)
	}
	if err := daemon.Start(baseDir, whisperDir); err != nil {
		otel.Error("start failed", map[string]any{"error": err.Error()})
		os.Exit(1)
	}
}
...
func runTranscribe(filePath string) {
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		otel.Error("file not found", map[string]any{"path": filePath})
		os.Exit(1)
	}

	if err := transcriber.CheckFFmpeg(); err != nil {
		otel.Error("ffmpeg not found")
		os.Exit(1)
	}

	if !transcriber.IsServerRunning() {
		otel.Error("server not running")
		os.Exit(1)
	}

	text, err := transcriber.Transcribe(filePath)
	if err != nil {
		otel.Error("transcribe failed", map[string]any{"error": err.Error()})
		os.Exit(1)
	}
	fmt.Println(text)
}
